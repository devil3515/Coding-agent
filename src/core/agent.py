...
class Agent:
    ...
    def chat(self, user_input: str, max_iterations: int = 10) -> str | None:
        ...
        while True:
            ...
            #-- KILL SWITCH: Prevent infinite tool loops -------------------------
            if iteration_count >= max_iterations:
                self.console.print("[bold red]⚠️ Max iterations reached. Forcing final response.[/bold red]")
                self.memory.add_message(Message(role="user", content="You have looped too many times. Please provide your final summary based on what you know."))
                context = self.memory.get_context()
                final_response = self.llm.complete(context, tools=None, max_tokens=self.llm_config.get("max_tokens", 6000))
            ...
            if self.working_directory and context[0].role == "system":
                context[0] = Message(
                    role="system",
                    content=f"{context[0].content}\n\n[SYSTEM NOTE]: You are currently operating in the directory: {self.working_directory}. You MUST use relative paths from this directory (e.g., 'src/main.py'). NEVER use absolute paths like /Users/name/... unless explicitly asked."
                )

            #--- INJECT ACTIVE PLAN ---------------------------------------------
            if self.current_plan and context[0].role == "system":
                active_idx = next(
            ...
            #-- ASK LLM ---------------------------------------------------------
            try:
                response = self.llm.complete(context, tools=self.registry.schemas, max_tokens=self.llm_config.get("max_tokens", 6000))
            ...
            #-- CHECK IF LLM IS DONE THINKING/ACTING -----------------------------
            ...
            #-- LLM WANTS TO USE TOOLS (OR GOT CUT OFF BY MAX_TOKENS LIMIT) -----
            else:
                ...
                for tool_call in response.tool_calls:
                    ...
                    # ==========================================
                        ...
                        for i, step in enumerate(self.current_plan):
                            files_hint = f"  →  {', '.join(step['files'])}" if step["files"] else ""
                            self.console.print(f"  [dim][ ] Step {i+1}:[/dim] {step['title']}{files_hint}")
                        self.console.print("")
                    ...
                    # ==========================================
                        ...
                        if q_type == "mcq" and args.get("options"):
                            ...
                            user_answer = self.console.input(f"\n[bold yellow]❓ AGENT QUESTION:[/bold yellow] {question}\n[bold blue]Your answer:> [/bold blue]")
                    ...
                    # ==========================================
                    # 5. NORMAL TOOL EXECUTION (Everything else)
                    # ==========================================
                    else:
