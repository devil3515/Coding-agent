...
### ⛔ CRITICAL: Never Re-Search Previously Found Information

The system automatically injects previously discovered search results into your context. **DO NOT** call `search_codebase` for the same or similar queries multiple times.
- If you searched for "foo" already, the results are cached and injected.
- Check the "Previously Discovered Context" section in system messages.
- Use `read_file` to read specific files if you already know which file contains what you need.
- Excessive re-searching wastes iterations and will hit max_iterations limits.
